...
_EMPLOYEE_STATUS_MESSAGES = {
    AccountStatus.APPROVED: ("Hisobingiz tasdiqlandi", "Kompaniyaga qo'shilish so'rovingiz tasdiqlandi. Endi buyurtma bera olasiz."),
    AccountStatus.REJECTED: ("So'rovingiz rad etildi", "Kompaniyaga qo'shilish so'rovingiz rad etildi."),
    AccountStatus.INACTIVE: ("Hisobingiz faolsizlantirildi", "Hisobingiz faolsizlantirildi."),
}
...
class CompanyAdminService:
    ...
    async def assign_kitchens(
        ...
        await self.get_branch(branch_id)  # filial shu kompaniyaniki ekanini tekshir
        kitchens: list[Kitchen] = []
        for kid in dict.fromkeys(kitchen_ids):  # dublikatsiz
            kitchen = await self.session.get(Kitchen, kid)
            if kitchen is None or kitchen.deleted_at is not None or not kitchen.is_active:
                raise NotFoundError(f"Oshxona topilmadi yoki faol emas: {kid}")
            kitchens.append(kitchen)
        await self.session.execute(
            delete(BranchKitchen).where(BranchKitchen.branch_id == branch_id)
        )
        for kitchen in kitchens:
            self.session.add(BranchKitchen(branch_id=branch_id, kitchen_id=kitchen.id))
        await self.session.commit()
        return kitchens
    ...
    async def bulk_confirm_orders(self) -> int:
        ...
        for order in orders:
            ...
            await notify(
                ...
                "Buyurtmangiz yetkazildi", "Bugungi buyurtmangiz yetkazildi.",
            )
        await self.session.commit()
        return len(orders)
    ...
    async def dashboard(self, year: int | None = None):
        ...
        summary = [
            await distinct_card(
                self.session, order_where, True, "lunch_subscribers_today",
                Order.employee_id, today, period_today(today),
            ),
            await revenue_card(
                self.session, order_where, True, "monthly_cost", today, period_month(today)
            ),
            await order_count_card(
                self.session, order_where, True, "delivered_total", today,
                period_year(year), OrderStatus.DELIVERED,
            ),
            await snapshot_card(
