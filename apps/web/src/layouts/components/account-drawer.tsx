...
export function AccountDrawer({ data = [], sx, ...other }: AccountDrawerProps) {
  ...
  const roleLabel = user?.role ? (ROLE_SHORT[user.role] ?? user.role) : '';
  ...
  return (
    <>
      <AccountButton
        onClick={onOpen}
        photoURL={user?.avatar_url ? getImagePreviewUrl(user.avatar_url) : undefined}
        ...
        <Scrollbar>
          ...
          >
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }} noWrap>
              {roleLabel}{user?.phone ? ` · ${user.phone}` : ''}
            </Typography>
          </Box>
