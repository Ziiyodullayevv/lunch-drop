...
type CarouselItemProps = BoxProps & { item: FeaturedItem };

function CarouselItem({ item, sx, ...other }: CarouselItemProps) {
  return (
    <Box sx={[{ width: 1, position: 'relative' }, ...(Array.isArray(sx) ? sx : [sx])]} {...other}>
      <Box
        sx={{
          p: 3,
          gap: 1,
      ...
      >
        ...
        <Typography variant="h5" noWrap>{item.title}</Typography>
